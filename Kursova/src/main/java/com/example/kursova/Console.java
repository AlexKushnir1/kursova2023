...
//        consoleLabel.setStyle("-fx-font-weight: bold;");
        ...
        outputTextArea.setStyle("-fx-control-inner-background: #E0FFFF; -fx-text-fill: black;");

        inputTextField = new TextField();
        inputTextField.setPrefWidth(800);
        inputTextField.setOnAction(event -> handleCommand());
        inputTextField.setStyle("-fx-control-inner-background: #E0FFFF; -fx-text-fill: black;");
        ...
        this.getChildren().addAll(closeButtonPane , outputTextArea, inputTextField);
    ...
    private void processDeletePlanetarySystemCommand(String line) {
        ...
        for (Bases sys : Bases.getListSystem()) {
    ...
    private void processDeleteNonPlanetarySystemObjectsCommand(String line) {
        ...
        for (Bases sys : Bases.getListSystem()) {
            if (sys.getName().equals(params)) {
                ...
                List<smok> objectsToRemove = new ArrayList<>();
    ...
    private void listObjectsInWorld(String line) {
        boolean found = false;
        List<smok> list = root.getListObj();
    ...
    private void processListObjectsInPlanetarySystemCommand(String line) {
        ...
        for (Bases sys : Bases.getListSystem()) {
            if (sys.getName().equals(params)) {
                ...
                List<smok> list = new ArrayList<>(sys.getListObjInSys());
    ...
    private void sortObjectsAlphabetically(List<smok> list) {
        list.sort(Comparator.comparing(smok::getName));
    }

    private void sortObjectsHealt(List<smok> list) {
        list.sort(Comparator.comparingDouble(smok::getHealth).reversed());
    }

    private void sortObjectSpead(List<smok> list) {
        list.sort(Comparator.comparingInt(smok::getMSpead).reversed());
    }
}
